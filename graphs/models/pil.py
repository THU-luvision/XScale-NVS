...
class CNet(nn.Module):
    def __init__(self, params):
        ...
        self.network_dir = nn.ModuleList([
            FiLMLayer(self.input_dim_dir, self.hidden_dim),
            FiLMLayer(self.hidden_dim, self.hidden_dim),
            # ----------------------------------------------
            # FiLMLayer(self.hidden_dim, self.hidden_dim),
            # ----------------------------------------------
        ])
        self.mapping_network_dir = CustomMappingNetwork(
            z_dim=self.hidden_dim, map_hidden_dim=self.params.descriptor_dim,
            map_output_dim=len(self.network_dir) * self.hidden_dim * 2
        )
        ...
        # self.layer_dl = nn.Sequential(nn.Linear(self.hidden_dim, 3), nn.Sigmoid())
        self.layer_sl = nn.Sequential(nn.Linear(self.hidden_dim, 3), nn.Sigmoid())
        # self.layer_dl = nn.Sequential(nn.Linear(self.hidden_dim, 3))
        # self.layer_sl = nn.Sequential(nn.Linear(self.hidden_dim, 3))
        
        # self.layer_dl.apply(frequency_init(25))
        self.layer_sl.apply(frequency_init(25))
        self.network.apply(frequency_init(25))
        self.network[0].apply(first_layer_film_sine_init)
        self.network_dir.apply(frequency_init(25))
        self.network_dir[0].apply(first_layer_film_sine_init)
    ...
    def forward(self, l_emb, x_emb, s_feat, n, v):
        ...
        # ref_dir = reflectance_direction(view_dir=v, normal=n)
        # frequencies_, phase_shifts_ = self.mapping_network_dir(torch.cat((s_feat, ref_dir), dim=-1))
        frequencies_, phase_shifts_ = self.mapping_network_dir(d_feat)
        frequencies_ = frequencies_ * 15 + 30
        y = v
        for index, layer in enumerate(self.network_dir):
            start = index * self.hidden_dim
            end = (index + 1) * self.hidden_dim
            y = layer(y, frequencies_[..., start:end], phase_shifts_[..., start:end])
        c = self.layer_sl(y)
