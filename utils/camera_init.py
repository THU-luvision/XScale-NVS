...
def select_group_pairs(projection_M, cameraTs, group_cameraTs, xyz_3D, cube_length, image_shape, angle_thres, group_pair_num_max, group_pair_num_min, group_pair_index):
    '''
...
def select_group(projection_M, cameraTs, group_cameraTs, xyz_3D, cube_length, image_shape, angle_thres, group_pair_num_max, group_pair_num_min):
    '''
    given group view number, select groupviews
    :param projection_M: the
        shape:(N_views, 3,4)
    ...
    if(images is not None):
        ...
        for iter in range(image_compress_stage):
            ...
            for image in images:
                ...
                image_resized = scipy.misc.imresize(image, size = (int(resized_h // (2**iter)), int(resized_w // (2**iter))), interp = 'bicubic')
                image_resized = image_resized/256.0 - 0.5
                image_resized_list.append(image_resized)
            images_resized = image_resized_list if return_list else np.stack(image_resized_list)
...
def generate_sparse(
    ...
    XYZ_id = 8 * ((_cube_D_/2) * (_cube_D_/2) * (XYZ[:, 0] // 2) + (_cube_D_/2) * (XYZ[:, 1] // 2) + XYZ[:, 2] // 2) + (
...
def generateMeta_from_xyz(projection_M,
    ...
    tau = project_cube_xyz[:, :, :, 0] / np.linalg.norm(project_cube_xyz[:, :, :,  0], axis=2)[:, :, None]  # shape:(N_view, N_points, 3)
