...
def readCameraRTK_as_np_tanks(cameraPO_file):
...
# TODO:
#  function for partitioned rendering.
def readCameraPOs_as_np_partitioned(
        datasetFolder,
        datasetName,
        poseNamePattern,
        # model,
    ...
    """
    NOTE:
        this implementation is currently for 16 parititions.
    inputs:
      datasetFolder: 'x/x/x/middlebury'
      datasetName: 'DTU' / 'Middlebury'
      #model: 1..128 / 'dinoxx'
    ...
    output:
      cameraPOs (N_views,3,4) np.flost64
    """
    partition_num = down_sample_rate ** 2
    cameraPOs = np.empty((len(viewList), partition_num, 3, 4), dtype=np.float64)
    cameraRTOs = np.empty((len(viewList), partition_num, 3, 4), dtype=np.float64)
    cameraKOs = np.empty((len(viewList), partition_num, 3, 3), dtype=np.float64)
    ## TODO:
    if 'Middlebury' in datasetName:
        cameraPOs = self.__readCameraPOs_as_np_Middlebury__(
            cameraPO_file=os.path.join(datasetFolder, poseNamePattern), viewList=viewList)
        ...
        for _i, _view in enumerate(viewList):
            ...
            _cameraRT, _cameraK = readCameraRTK_as_np_tanks(cameraPO_file=os.path.join(datasetFolder,
                                                                                       poseNamePattern.replace('#',
                                                                                           ...
                                                                                           '@',
                                                                                           '{}'.format(_view - 1))))
            # TODO: function for partitioning _cameraK:
            _cameraKs = K_partition(_cameraK, down_sample_rate, img_size)
            # cameraKOs[_i] = _cameraKs
            # cameraRTOs[_i] = np.repeat(np.expand_dims(_cameraRT, axis=0), partition_num, axis=0)
            # cameraPOs[_i] = np.dot(_cameraKs, keep_dims=...)
            for _j in range(partition_num):
                cameraKOs[_i][_j] = _cameraKs[_j]
                cameraRTOs[_i][_j] = _cameraRT
                cameraPOs[_i][_j] = np.dot(_cameraKs[j], _cameraRT)

    # print('cameraPOs', cameraPOs)
    return cameraPOs, cameraRTOs, cameraKOs

def K_partition(cameraKO, down_sample_rate=4, img_size=(1200, 1600)):
    cx = cameraKO[0][2]
    cy = cameraKO[1][2]
    ...
    h = int(img_size[0] / down_sample_rate)
    w = int(img_size[1] / down_sample_rate)
    for i in range(down_sample_rate):
        for j in range(down_sample_rate):
            cxx = cx - j * w
            cyy = cy - i * h
...
def readCameraP0s_np_allModel_partitioned(datasetFolder,
                              datasetName,
    ...
    for i in modelList:
        if 'tanks' in datasetName:
            ##########TODO###################

            cameraPOs, cameraRTOs, cameraKOs = readCameraPOs_as_np_partitioned(datasetFolder,
                                                                   datasetName,
                                                                   poseNamePattern,
                                                                   viewList, down_sample_rate,
                                                                   img_size)
        ...
        elif 'DTU' in datasetName:
            cameraPOs, cameraRTOs, cameraKOs = readCameraPOs_as_np_partitioned(datasetFolder,
                                                                               datasetName,
                                                                               poseNamePattern,
                                                                               viewList, down_sample_rate,
                                                                               img_size)
            ones = np.repeat(np.array([[[0, 0, 0, 1]]]), repeats=(cameraPOs.shape[0] * cameraPOs.shape[1]), axis=0)
            cameraP0s = np.concatenate((cameraPOs, ones), axis=2)
...
def cameraPs2Ts_all_partitioned(cameraPOs_all):
    """
        returns:
            cameraT_all (N_model, N_view, num_partition, 3)

    """
    model_num = len(cameraPOs_all)
    view_num = cameraPOs_all[0].shape[0]
    partition_num = cameraPOs_all[0].shape[1]
    # pdb.set_trace()
    cameraT_all = np.zeros((model_num, view_num, partition_num, 3))
    for i in range(model_num):
        for j in range(view_num):
            cameraT_all[i][j] = cameraPs2Ts(cameraPOs_all[i][j])
...
def CameraPOs_as_torch_partitioned(cameraPO4s_model, cameraRTO4s_model, cameraKO4s_model,
                                   down_sample_rate=4, img_size=(1200, 1600)):
    '''
    Args:
    ...
    '''
    cameraPO4s_model = cameraPO4s_model.numpy()
    cameraRTO4s_model = cameraRTO4s_model.numpy()
    cameraKO4s_model = cameraKO4s_model.numpy()

    print('cameraKO4s_models shape: ', cameraKO4s_model.shape)
    print('cameraRTO4s_models shape: ', cameraRTO4s_model.shape)
    print('cameraPO4s_models shape: ', cameraPO4s_model.shape)

    # num_model = len(cameraKO4s_models)
    num_view = cameraPO4s_model.shape[0]
    num_partition = down_sample_rate ** 2

    # print('num_model: ', num_model)
    print('num view: ', num_view)
    print('num_partition: ', num_partition)

    # TODO: modify the dimension from (3,4) to (4,4)
    ...
    for j in range(num_view):
        ...
        cameraK0s = K_partition(cameraK0, down_sample_rate, img_size) # (num_partition, 3, 3)
...
def partition_image_and_matrix(images,
    ...
    '''
    num_view = len(images)
    num_partition = int(compress_ratio ** 2)
    parti_h = int(images[0].shape[1] / compress_ratio)
    parti_w = int(images[0].shape[2] / compress_ratio)
    ...
    for view_i in range(num_view):
        for partition_j in range(num_partition):
            ...
            start_h_idx = math.floor(partition_j / compress_ratio)
            start_w_idx = (partition_j % compress_ratio)
            ...
            parti_imgs[view_i, partition_j, :, :, :] = images[view_i][:, start_h_pix: final_h_pix, start_w_pix: final_w_pix]
    # print('^^^^^^^^^^', parti_imgs.shape)

    _cameraP04s, _, _ = CameraPOs_as_torch_partitioned(cameraPO4s_model, cameraRTO4s_model, cameraKO4s_model,
                                                 down_sample_rate=compress_ratio,
